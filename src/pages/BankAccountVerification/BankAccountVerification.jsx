...
const BankAccountVerification = () => {
  ...
  const [ifseCode, setIfscCode] = useState('')
  ...
  const Submit = () => {
    ...
    setIsloading(true)
    let config = {
      url: ApiUrl?.bankAccountVerify,
      method: "post",
      body: {
        "account_number": bankAccountNo,
        "ifsc_code": ifseCode.toUpperCase()
      }
    }
    APIRequest(
      config,
        ...
        <div className="comman-container px-4">
          <div className='button-process'>
            <div style={{ maxWidth: 400, margin: 'auto' }}>
              <InputFeild
                type="number"
                PlaceHolderTitle='Bank account number'
                InputFeildValue={bankAccountNo}
                setInputFeildValue={setBankAccountNo}
                disc={'Please enter bank account no.'}
              />
              <InputFeild type="text"
                PlaceHolderTitle='IFSC CODE'
                InputFeildValue={ifseCode}
                setInputFeildValue={setIfscCode}
                disc={'Please enter ifsc code'}
                uppercase='uppercase'
              />
