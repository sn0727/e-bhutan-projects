...
export const RegistrationOnBoarding = ({ children, getTokenData, invoiceNo }) => {
  ...
  const MerchantBoarding = async () => {
    ...
    let location_latitude = await sessionStorage.getItem('location_ip')
    let location_longitude = await sessionStorage.getItem('location_ip')
...
export const AepsServices = ({ children, getTokenData, invoiceNo, savePaymentOption }) => {
  const [isLoading, setIsloading] = useState(false);
  const [FingerData, setFingerData] = useState('')
  const [getLocationData, setGetLocationData] = useState({});
...
const Aeps = () => {
  ...
  if (getTokenData?.isAEPS === 'false') {
    ...
    return (
      <>
        <Header />
        <AepsServices getTokenData={getTokenData} invoiceNo={invoiceNo} savePaymentOption={savePaymentOption} >
          <SelectBiometricDeviceTab savePaymentOption={savePaymentOption} setSavePaymentOption={setSavePaymentOption} />
        </AepsServices>
        <Footer />
      </>
