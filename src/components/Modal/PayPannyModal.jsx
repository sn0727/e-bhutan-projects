...
const PayPannyModal = ({ itemData, mobileNo, fetchBeneficiaryFun }) => {
    ...
    const transferMoneyDropPenny = () => {
        setisLoading(true)
        let config = {
            url: ApiUrl?.pennyDrop,
            method: 'post',
            body: {
                "mobile": mobileNo,
                "accno": itemData?.accno,
    ...
    return (
        <>
            ...
            {itemData?.verified === "1" ? null : <Button onClick={onOpen} className='FaChevronRight' colorScheme='blue' marginRight={'10px'}>
                Pay 1&#8377; For Test
            </Button>}
                        ...
                        <ChakraProvider>
                            <div className="comman-container px-4">
                                <div className='Verified-Transfer' style={{ marginTop: '60px' }}>
                                    <p>Transfering To : - {itemData?.name}
                                    </p>
                                    <p>A/c no :- {itemData?.accno} </p>
                                    <p>IFSC Code :- {itemData?.ifsc} </p>
                                    <p>A/c Type :- NEFT</p>

                                    <strong className='indianRupes'>1 <sub>rs</sub></strong>
